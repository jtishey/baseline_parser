...
class Run(object):
    ...
    def execute_ping_check(self):
        ...
        ping_vars = {'JUNOS': {'iterword': 'Success', 'match_index': 6},
                     'TiMOS': {'iterword': 'Success', 'match_index': 6},
                     'IOS': {'iterword': 'packets', 'match_index': 3},
                     'XR': {'iterword': 'packets', 'match_index': 3}}
        ...
        for after_line in self.after_cmd_output:
            if iterword in after_line:
                ...
                break
        if found_match is False:
            print("FAILED! " + self.test_values[0]['command'] + " not found in the after baseline")
            self.ping_totals['FAIL'] += 1
            return
        # Standardize output across platforms
        if self.device.os_type == 'JUNOS' or self.device.os_type == 'TiMOS':
            success_rate = str(100 - int(line[match_index].replace('%', '')))
        else:
        ...
        if line[match_index] == after_line[match_index]:
            if self.device.config.verbose is True:
                print("PASSED! " + self.test_values[0]['command'] + " " + success_rate + \
                     '% Success before and after')
            self.ping_totals['PASS'] += 1
        else:
            print("FAILED! " + self.test_values[0]['command']  + " pre=" + \
